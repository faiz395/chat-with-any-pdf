...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      console.log('📤 Uploading file to Appwrite bucket...');
      const uploadedFile = await serviceServer.addNewPdfToBucket(file);
      ...
      return new NextResponse(
        JSON.stringify({ error: 'Failed to upload file to bucket' }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
        }
